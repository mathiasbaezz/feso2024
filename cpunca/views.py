...
def index(request):
    ...
    return render(request, 'feso.html', {'posts':posts})

def raiz(request):
    return redirect('/feso')
...
def tabla_campeones_unca(request):
    ...
    return render(request, 'tabla_campeonesfeso.html', {'posts':posts, 'campeones': campeones, 'champions': champions})
...
def mostrar_partidos_unca(request):
    ...
    return render(request, 'partidosfeso.html', {'partidouncas': partidos, 'posts':posts})
...
def resumen_unca(request, pk):
    ...
    return render(request, 'resumenfeso.html', context)


def deportes_unca (request):
    posts = Post.objects.all().order_by('-published_date')[0:4]
    deportes = DeporteUnca.objects.all()
    return render(request, 'feso.html', {"posts": posts, "deportes":deportes})
...
def champions_unca (request):
    ...
    return render(request, 'tabla_campeonesfeso.html', {'posts':posts, "champions":champions, "championsgral":championsgral})
...
def actualizar_tabla_posiciones(request):
    ...
    return render(request, 'tabla_posicionesfeso.html', {
